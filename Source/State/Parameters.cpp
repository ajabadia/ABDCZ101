    ...
    : audioProcessor(processor)
{
    ...
    lineSelect   = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::lineSelect));
    osc1Waveform = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::osc1Waveform));
    osc1Waveform2= dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::osc1Waveform2));
    osc1Level    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::osc1Level));
    osc2Waveform = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::osc2Waveform));
    osc2Waveform2= dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::osc2Waveform2));
    osc2Level    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::osc2Level));
    osc2Detune   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::osc2Detune));
    
    // Audit Fix [2.4]
    lineMix      = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::lineMix));

    detuneOct    = dynamic_cast<juce::AudioParameterInt*>(apvts->getParameter(ParameterIDs::detuneOct));
    detuneCoarse = dynamic_cast<juce::AudioParameterInt*>(apvts->getParameter(ParameterIDs::detuneCoarse));
    detuneFine   = dynamic_cast<juce::AudioParameterInt*>(apvts->getParameter(ParameterIDs::detuneFine));
    hardSync     = dynamic_cast<juce::AudioParameterBool*>(apvts->getParameter(ParameterIDs::hardSync));
    ringMod      = dynamic_cast<juce::AudioParameterBool*>(apvts->getParameter(ParameterIDs::ringMod));
    noiseMod     = dynamic_cast<juce::AudioParameterBool*>(apvts->getParameter(ParameterIDs::noiseMod)); // Audit Fix
    glideTime    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::glideTime));
    
    lfoWaveform  = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::lfoWaveform));
    lfoRate      = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::lfoRate));
    lfoDepth     = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::lfoDepth));
    lfoDelay     = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::lfoDelay));

    driveAmount  = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::driveAmount));
    driveColor   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::driveColor));
    driveMix     = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::driveMix));

    chorusRate   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::chorusRate));
    chorusDepth  = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::chorusDepth));
    chorusMix    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::chorusMix));
    delayTime    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::delayTime));
    delayFeedback= dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::delayFeedback));
    delayMix     = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::delayMix));
    reverbSize   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::reverbSize));
    reverbMix    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::reverbMix));
    
    dcaAttack    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::dcaAttack));
    dcaDecay     = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::dcaDecay));
    dcaSustain   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::dcaSustain));
    dcaRelease   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::dcaRelease));
    
    dcwAttack    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::dcwAttack));
    dcwDecay     = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::dcwDecay));
    dcwSustain   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::dcwSustain));
    dcwRelease   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::dcwRelease));
    
    protectSwitch = dynamic_cast<juce::AudioParameterBool*>(apvts->getParameter(ParameterIDs::protectSwitch));
    systemPrg     = dynamic_cast<juce::AudioParameterBool*>(apvts->getParameter(ParameterIDs::systemPrg));
    masterVolume   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::masterVolume));
    hardwareNoise  = dynamic_cast<juce::AudioParameterBool*>(apvts->getParameter(ParameterIDs::hardwareNoise));
    operationMode  = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::operationMode));

    modernLpfCutoff = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::lpfCutoff));
    modernLpfReso   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::lpfReso));
    modernHpfCutoff = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::hpfCutoff));

    modVeloToDcw     = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::modVeloDcw));
    modVeloToDca     = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::modVeloDca));
    modWheelToDcw    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::modWheelDcw));
    modWheelToLfoRate = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::modWheelLfoRate));
    modWheelToVibrato = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::modWheelVib));
    modAtToDcw       = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::modAtDcw));
    modAtToVibrato   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::modAtVib));

    keyTrackDcw      = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::keyTrackDcw));
    keyTrackPitch    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::keyTrackPitch));
    keyFollowDco     = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::keyFollowDco));
    keyFollowDcw     = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::keyFollowDcw));
    keyFollowDca     = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::keyFollowDca));

    midiChannel      = dynamic_cast<juce::AudioParameterInt*>(apvts->getParameter(ParameterIDs::midiChannel));
    masterTune       = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::masterTune));
    pitchBendRange   = dynamic_cast<juce::AudioParameterInt*>(apvts->getParameter(ParameterIDs::benderRange));
    keyTranspose     = dynamic_cast<juce::AudioParameterInt*>(apvts->getParameter(ParameterIDs::transpose));
    
    // Phase 5.1: Oversampling
    oversamplingQuality = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::oversampling));

    // Arpeggiator
    arpEnabled = dynamic_cast<juce::AudioParameterBool*>(apvts->getParameter(ParameterIDs::arpEnabled));
    arpLatch   = dynamic_cast<juce::AudioParameterBool*>(apvts->getParameter(ParameterIDs::arpLatch));
    arpRate    = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::arpRate));
    arpBpm     = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::arpBpm));
    arpGate    = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::arpGate));
    arpSwing   = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::arpSwing));
    arpSwingMode = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::arpSwingMode));
    arpPattern = dynamic_cast<juce::AudioParameterChoice*>(apvts->getParameter(ParameterIDs::arpPattern));
    arpOctave  = dynamic_cast<juce::AudioParameterInt*>(apvts->getParameter(ParameterIDs::arpOctave));

    // Macros
    macroBrilliance = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::macroBrilliance));
    macroTone       = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::macroTone));
    macroSpace      = dynamic_cast<juce::AudioParameterFloat*>(apvts->getParameter(ParameterIDs::macroSpace));
